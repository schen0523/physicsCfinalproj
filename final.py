...
def preset1(button):
    ...
    mass2slider.value = 1.1727; updatemass2(mass2slider)
...
def preset2(button):
    mass1slider.value = 1.0; updatemass1(mass1slider)
    mass2slider.value = 16; updatemass2(mass2slider)
    string1slider.value = 5.0; updatestring1(string1slider)
    string2slider.value = 5.0; updatestring2(string2slider)
    angleslider.value = pi/2; updateangle(angleslider)
    reset(None)
...
def updatepradius(slider):
    ...
    last_changed = "rpivot"
    if labels:
        pradius1label.text = f"{rpivot:.3f} m "
        pradius2label.text = f"{rpivot:.3f} m "
...
scene.append_to_caption("\n\t")
wtext(text="1.0 kg  ")
mass1slider = slider(min=1.0, max=30.0, value=1.0, length=300, bind=updatemass1)
wtext(text="  30.0 kg")
scene.append_to_caption("\n\n")
...
scene.append_to_caption("\n\t")
wtext(text="1.0 kg  ")
mass2slider = slider(min=1.0, max=30.0, value=1.0, length=300, bind=updatemass2)
wtext(text="  30.0 kg")
scene.append_to_caption("\n\n")
...
pivot1_center = vector(-2.5, 2.5, 0)
pivot2_center = vector(5, 2.5, 0)
pivot1 = cylinder(pos=pivot1_center - vector(0,0,0.05), axis=vector(0, 0, 0.1), radius=0.1, color=color.white)
pivot2 = cylinder(pos=pivot2_center - vector(0,0,0.05), axis=vector(0, 0, 0.1), radius=0.1, color=color.white)
bridge_string = curve(pos=[pivot2.pos, pivot1.pos], color=vec(0.7, 0.9, 0.95))

mass1 = sphere(pos=vector(-2.5, -2.5, 0), color=color.blue, radius=0.5, make_trail=False)
mass1label = label(pos=mass1.pos, text="1.0 kg", box=False)

mass2 = sphere(pos=vector(5, -2.5, 0), color=color.red, radius=0.5, make_trail=False)
...
pradius1label = label(pos=pivot1.pos, text="0.0 m", xoffset=30, yoffset=15, box=False)
pradius2label = label(pos=pivot2.pos, text="0.0 m", xoffset=30, yoffset=15, box=False)
...
def update_visuals():
    bottom1 = pivot1_center + vector(l1 * sin(-theta), -l1 * cos(-theta), 0)
    ...
    bottom2 = pivot2_center + vector(0, -(L - l1), 0)
    mass2.pos = bottom2
